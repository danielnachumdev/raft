...
**Doctor groups:** built-in group `raft` (edge + host checks); App `spec.group` (at most one). Ungrouped apps have no heading. Member labels are Compose service ids: ungrouped `{name}`, grouped `{group}-{name}`; edge is `raft-gate` / `raft-router` (containers `raft-{service}-1` via project `raft`).
