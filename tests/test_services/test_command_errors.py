...
from raft.services.command_errors import (
...
from raft.services.git_errors import (
...
class TestValidationCTAs(RaftTestCase):
    ...
    def test_settings_bad_ports_and_shapes(self) -> None:
        ...
        path.write_text("edge:\n  streams:\n    - {name: s, port: nope}\n", encoding="utf-8")
        with pytest.raises(ValueError, match="streams\\['s'\\].port"):
            load_config(self.tmp_path)
    ...
    def test_ports_require_int_and_bool(self) -> None:
        with pytest.raises(ValueError, match="containerPort must be an integer"):
            parse_ports(
                {"ports": [{"name": "http", "containerPort": "eighty", "expose": "http"}]},
                Path("app.yaml"),
            )
        with pytest.raises(ValueError, match="proxyProtocol must be a boolean"):
            parse_ports(
                {
                    "ports": [
                        {
    ...
    def test_wait_until_fix_cta(self) -> None:
        with pytest.raises(TimeoutError, match="Fix: retry"):
    ...
    def test_settings_unreadable(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with pytest.raises(ValueError, match="cannot read settings.yaml"):
            load_config(self.tmp_path)
